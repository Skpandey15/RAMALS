...
CREATE TABLE ledger.learning_workflow_run (
  id UUID PRIMARY KEY,
...
CREATE INDEX idx_learning_workflow_learner
  ON ledger.learning_workflow_run (learner_id, started_at DESC);

CREATE INDEX idx_learning_workflow_running
  ON ledger.learning_workflow_run (deadline_at)
  WHERE status = 'RUNNING';

CREATE TABLE ledger.learning_workflow_step (
  id UUID PRIMARY KEY,
  run_id UUID NOT NULL REFERENCES ledger.learning_workflow_run(id) ON DELETE RESTRICT,
  ...
  CONSTRAINT uq_learning_workflow_step_index UNIQUE (run_id, step_index),

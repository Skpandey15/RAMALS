...
public enum SelectionReason {
  ...
  /** DIAGNOSTIC_SELECTION_V4 only -- see HypothesisConfirmationDiagnosticSelector. This skill's
   * two most recent mastery snapshots show a status regression (the previous one was better than
   * the latest), a cross-attempt signal that something unexpected happened -- a fluke miss, or a
   * real, newly-surfaced gap. Cross-attempt because the current one-shot batch-submit model has no
   * way to ask a follow-up within the same attempt; this is that follow-up, deferred to the
   * learner's next diagnostic session rather than never asked at all. Confirming or refuting it is
   * given priority over routine coverage; the band tested is whatever the skill's own signal (held,
   * possibly further capped by an unsecured prerequisite) already decided -- this reason changes
   * priority and its own label, never the band. */

...
public class LearningWorkflowOrchestrator {
  ...
  public Run trigger(EvaluationTrigger trigger) {
    ...
    BigDecimal score =
        eligibility.eligible() ? trigger.normalizedScore() : BigDecimal.ZERO;
   ...
   *
   * <p>Safe to call repeatedly and from more than one caller: every terminal transition is guarded
   * on the run still being RUNNING, so the first outcome a run reaches is the one it keeps.
   */
  public Run advance(UUID runId) {
    ...
    int priorAttempts = runs.step(runId, step).map(StepRun::attemptCount).orElse(0);
    if (!LearningWorkflowPolicy.mayRetry(priorAttempts)) {
      fail(run, step, REASON_ATTEMPTS_EXHAUSTED);
      return reload(runId);
    }

    // Commits the attempt before any work begins. This ordering is the loop bound (G06).
    runs.beginStep(runId, step);
    try {
      apply(run, step, execute(run, step));
      ...
      LOGGER
          ...
          .addKeyValue("attempt", priorAttempts + 1)
          .addKeyValue("errorType", failure.getClass().getSimpleName())
          .log("workflow step failed", failure);
      if (LearningWorkflowPolicy.mayRetry(priorAttempts + 1)) {
        runs.retryStep(runId, step, REASON_STEP_FAILED);
      } else {
        fail(run, step, REASON_ATTEMPTS_EXHAUSTED);
      }
    }
  ...
  public boolean cancel(UUID runId, String reasonCode) {
    ...
    expire(run, Status.CANCELLED, reasonCode == null ? REASON_CANCELLED : reasonCode);
    return true;
  }
  ...
  private StepOutcome runAdaptation(Run run) {
    WorkflowAgentStep.Result result = adaptation.adapt(run);
    if (!result.succeeded()) {
      throw new WorkflowStepFailedException(result.reasonCode());
    }
  ...
  private void apply(Run run, Step step, StepOutcome outcome) {
    runs.finishStep(
        run.id(), step, outcome.status(), outcome.reasonCode(), outcome.requestId(),
        outcome.resultRef());
    ...
    if (!outcome.continuation().eligible()) {
      ...
      skipRemaining(run, step, outcome.continuation().reasonCode());
      runs.finishRun(run.id(), Status.STOPPED, outcome.continuation().reasonCode());
      log("workflow.stopped", run, step, outcome.continuation().reasonCode());
      return;
    }

    Optional<Step> next = LearningWorkflowPolicy.next(step);
    if (next.isEmpty()) {
      runs.finishRun(run.id(), Status.COMPLETED, REASON_COMPLETED);
      log("workflow.completed", run, step, REASON_COMPLETED);
      return;
    }
  ...
  private void skipRemaining(Run run, Step from, String reasonCode) {
    ...
    while (next.isPresent()) {
      Step step = next.get();
      runs.beginStep(run.id(), step);
      runs.finishStep(run.id(), step, StepStatus.SKIPPED, reasonCode, null, null);
      next = LearningWorkflowPolicy.next(step);
    }
  }

  private void fail(Run run, Step step, String reasonCode) {
    runs.finishStep(run.id(), step, StepStatus.FAILED, reasonCode, null, null);
    runs.finishRun(run.id(), Status.FAILED, reasonCode);
    log("workflow.failed", run, step, reasonCode);
  }
  ...
  /**
   * Closes a run that ran out of time or was cancelled, marking whichever step it was on. The step
   * carries the same terminal status as the run so a reader is never left guessing which step the
   * clock caught.
   */
  private void expire(Run run, Status status, String reasonCode) {
    ...
    if (step != null) {
      ...
      runs.beginStep(run.id(), step);
      runs.finishStep(run.id(), step, stepStatus, reasonCode, null, null);
      skipRemaining(run, step, reasonCode);
    }
    log("workflow." + status.name().toLowerCase(java.util.Locale.ROOT), run, step, reasonCode);
  }

...
| **TD-T18-02 — agent proposal quality is unvalidated end to end** | 🟡 accepted for MVP-1 | Tutor and Adaptation both return `UNPROCESSABLE_PROPOSAL` on the `ci-fake` model route: the deterministic fake returns canned output that cannot satisfy the plane's validators, by design. Every `ai_execution` row from the M1-T18 run therefore records `AI_UNAVAILABLE`. | What is proven is that the agent is reachable from the learner journey, that deterministic authority is preserved when it cannot answer, and that the failure is durably recorded with provenance — which is what the release gate asks. What is not proven is that a real model produces a valid, useful proposal. Closing it needs a run with a real `RAMALS_AI_MODEL_ROUTE` and a provider credential, and should happen before MVP-1 is called production-ready. |

...
| **TD-T18-02 — adaptation chain proven to the AI plane, not through the proposal gate** | ✅ CLOSED 2026-08-22 | Every link of `learner event → deterministic recommendation → commit → AFTER_COMMIT listener → AdaptationService → ramals-ai → AdaptationProposalGate → `ai_execution`` is proven except the gate: the plane returns `UNPROCESSABLE_PROPOSAL` and `AdaptationService` catches it first. The cause is the shipped deterministic route, not a defect — `FakeProvider` returns a plain string rather than JSON, so every agent validator rejects it by design. `agent_run_id`, `prompt_template_id`, `prompt_version` and `model_route` are therefore NULL on every row. | Accepted because the agent is provably reachable from a real learner journey, the deterministic recommendation is unaffected by the agent failing, the failure is durably recorded with correlation, the gate's own logic is unit-tested, and no candidate defect is implicated. Closed on v0.1.0-rc8 by a live-provider run: `model_route=adaptation-default`, provider OpenAI, concrete model `gpt-4.1-2025-04-14`, resolved table `ROUTE_TABLE_V1+adaptation-default:model=gpt-4.1-2025-04-14`. Five executions on one learner submission, all `SUCCEEDED` with `error_code` NULL, and `agent_run_id`, `prompt_template_id`, `prompt_version` and `model_route` populated on every row — the four columns this item recorded as NULL. The gate and its validators were not weakened and the candidate was not rebuilt. Credential supplied from the User-scope environment only and never persisted; the plane was returned to `ci-fake` afterwards. Evidence: `docs/release/evidence/td-t18-02-closure.md`. `model_id` remains NULL — tracked separately as TD-RC8-02. |
...
| **TD-RC8-02 — ai_execution cannot establish which concrete model produced a response** | 🟡 open, provenance/auditability debt | `ai_execution.model_id` is NULL, including on the successfully qualified live-provider execution that closed TD-T18-02. `model_route` identifies only the *logical* route, and since RC8 a route has more than one approved vendor behind it — `claude-sonnet-5` and `gpt-4.1-2025-04-14` are both approved on every live route, selectable at runtime through `RAMALS_AI_MODEL_PINS`. The persisted execution record therefore cannot independently establish which provider or model produced a given response. The information exists at call time: the resolved route table version names the model, and the plane logs the provider. Neither is written to the row, so reconstructing it means correlating against logs that rotate. | Persisting immutable routing provenance on the execution record — at minimum the resolved provider, the resolved `model_id`, the route identifier, and the route/configuration version or equivalent. Deliberately not implemented during RC8 qualification: no MVP-1 release criterion requires concrete-model provenance, and TD-T18-02's own closure condition names `model_route` rather than `model_id`, so this is a gap that RC8 made *material* rather than one RC8 introduced. It should be settled before any evaluation or incident analysis depends on attributing an output to a vendor. |

...
| Database implementation records stopped at M0-T06 | Migrations continued to V023. The schema is documented in the migrations and ADRs, so nothing is undocumented — but `docs/database/` no longer indexes it. Resume the practice or retire it deliberately. |

...
| isolated PostgreSQL: `.\gradlew.bat :learning-platform:integrationTest --tests '*PostgresMigrationIntegrationTests' --rerun-tasks` | PASS — 14 tests, 0 failures, 0 skipped; verifies empty install and all 30 forward migrations |
| `scripts/ci/check-migration-compatibility.py` | PASS — all 30 migrations remain safe for the previously released image |

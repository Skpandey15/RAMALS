...
bounded deterministic candidate probes     (V5/H4b resolution only; no M2-ADR-032 proposal enters here)
        ->
HYPOTHESIS_DISCRIMINATION_V1               (stage 8 — implemented, inert; M2-ADR-034 Amendment 2)
        ->
...
- **NEXT** — a learner-scoped misconception-graph overlay and adaptive misconception-driven probing
  (each its own ADR per M2-ADR-033 §5/§6); designing and ratifying `DIAGNOSTIC_SELECTION_V6`
  (M2-ADR-034 Step 3, still design-only, the immediate next milestone that would wire
  `HYPOTHESIS_DISCRIMINATION_V1` into selection); the M2-ADR-032 advisory probe-proposal contract; a

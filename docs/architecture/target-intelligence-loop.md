...
deterministic score / ranking                        (score DESC -> hypothesis canonical order -> probe id)
        ->
DIAGNOSTIC_SELECTION_V6   [implemented 2026-09-11]    (stage 9 — Step 3; runtime semantics ratified
                                                        by Amendment 3, implemented exactly to spec)
...
- **NOW** — evidence, mastery + confidence, prerequisite graph, misconception entity/evidence/
  ...
  (`V1`–`V6`), new-evidence capture, deterministic mastery recompute; the inert
  `HYPOTHESIS_UNCERTAINTY_V1` (M2-ADR-034 step 1) and `HYPOTHESIS_DISCRIMINATION_V1` (step 2)
  foundations; and `DIAGNOSTIC_SELECTION_V6` (M2-ADR-034 step 3, implemented 2026-09-11 per
  Amendment 3), wiring `HYPOTHESIS_DISCRIMINATION_V1` into `V5`'s tiebreak with a fallback to `V5`'s
  own unmodified selection.
- **NEXT** — a learner-scoped misconception-graph overlay and adaptive misconception-driven probing
  (each its own ADR per M2-ADR-033 §5/§6); the M2-ADR-032 advisory probe-proposal contract; a

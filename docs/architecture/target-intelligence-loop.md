...
- **NEXT** — a learner-scoped misconception-graph overlay and adaptive misconception-driven probing
  (each its own ADR per M2-ADR-033 §5/§6); `INFORMATION_GAIN_V1` and `DIAGNOSTIC_SELECTION_V6`
  (M2-ADR-034 Steps 2–3, still design-only, the immediate next milestones); the M2-ADR-032 advisory
  probe-proposal contract; a time-bound goal + trajectory model; a typed goal-relative gap taxonomy;
  the still-unwritten M2-ADR-022.
...
- M2-ADR-034 — stages 7 and 8. Amendment 1 (2026-09-10) ratified `HYPOTHESIS_UNCERTAINTY_V1` (stage
  7); step 1 (2026-09-11) implements it, inert and unwired. `INFORMATION_GAIN_V1` (Step 2) and
  `DIAGNOSTIC_SELECTION_V6` (Step 3, stage 8) remain design-only.

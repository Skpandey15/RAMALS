...
diagnostic hypotheses
        ->
HYPOTHESIS_UNCERTAINTY_V1                  (stage 7 — implemented, inert; M2-ADR-034 Amendment 1)
        ->
bounded deterministic candidate probes     (V5/H4b resolution only; no M2-ADR-032 proposal enters here)
        ->
HYPOTHESIS_DISCRIMINATION_V1               (stage 8 — mathematics ratified, implementation pending;
                                             M2-ADR-034 Amendment 2)
        ->
deterministic score / ranking              (score DESC -> hypothesis canonical order -> probe id)
        ->
DIAGNOSTIC_SELECTION_V6   [future]         (stage 9 — design-only, Step 3; not authorized here)
```
...
- **NOW** — evidence, mastery + confidence, prerequisite graph, misconception entity/evidence/
  ...
  `HYPOTHESIS_UNCERTAINTY_V1` foundation (M2-ADR-034 step 1, implemented, unwired).
- **NEXT** — a learner-scoped misconception-graph overlay and adaptive misconception-driven probing
  (each its own ADR per M2-ADR-033 §5/§6); implementing the now-ratified `HYPOTHESIS_DISCRIMINATION_V1`
  (M2-ADR-034 Step 2, the immediate next milestone) and, after that, `DIAGNOSTIC_SELECTION_V6`
  (Step 3, still design-only); the M2-ADR-032 advisory probe-proposal contract; a time-bound goal +
  trajectory model; a typed goal-relative gap taxonomy; the still-unwritten M2-ADR-022.
...
- M2-ADR-034 — stages 7 and 8. Amendment 1 (2026-09-10) ratified `HYPOTHESIS_UNCERTAINTY_V1` (stage
  ...
  `HYPOTHESIS_DISCRIMINATION_V1` (stage 8, Step 2 mathematics; implementation pending).
  `DIAGNOSTIC_SELECTION_V6` (Step 3, stage 8) remains design-only.

...
- **NOW** — evidence, mastery + confidence, prerequisite graph, misconception entity/evidence/
  ...
  (`V1`–`V5`), new-evidence capture, deterministic mastery recompute; plus the active documentation
  step: M2-ADR-034.
- **NEXT** — a learner-scoped misconception-graph overlay and adaptive misconception-driven probing
  (each its own ADR per M2-ADR-033 §5/§6), explicit hypothesis posterior + information-gain selection
  (M2-ADR-034 impl), the M2-ADR-032 advisory probe-proposal contract, a time-bound goal + trajectory
  model, a typed goal-relative gap taxonomy, the still-unwritten M2-ADR-022.
...
- M2-ADR-034 — active decision for stages 7 and 8 (design only).

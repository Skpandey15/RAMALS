...
| 6c | **Misconception graph edges + queryable graph surface** | edges `IMPLEMENTED` (foundation: persistence + domain model + authored validation) / query surface `DESIGNED` | edges `NOW`; query surface + adaptive use `NEXT` | **M2-ADR-033**: `core.misconception_relationship` + `core.misconception_prerequisite_link` (`V061`, step 1); `MisconceptionRelationshipService` / `Validator` / `Repository` (`io.ramals.learningplatform.assessment.misconceptiongraph`) | Step 1 stores and deterministically validates authored, non-authoritative `MISCONCEPTION_RELATED` (misconception↔misconception, sub-types `CO_OCCURS_WITH` / `SPECIALISES` / `CONTRASTS_WITH`, symmetric ones canonically ordered, `SPECIALISES` acyclic) and `MISCONCEPTION_PREREQUISITE_LINK` (misconception→prerequisite `core.skill`) edges, `DRAFT`/`PUBLISHED` immutable, published-endpoint-only. **No** graph traversal/query surface, adaptive selection, information gain, `DIAGNOSTIC_SELECTION_V6`, or AI graph authoring -- edges are stored and validated only and no selector reads them. Extends M2-ADR-026 §8. |

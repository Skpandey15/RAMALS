...
| [M2-ADR-026](M2-ADR-026-diagnostic-confidence-v1.md) | `DIAGNOSTIC_CONFIDENCE_V1`: confidence attaches to the (learner, source objective, target objective, relationship type) hypothesis tuple, computed over every distinct `core.diagnostic_probe_provenance`-referenced evidence observation classified by H4b's own unmodified `HypothesisEvidenceOutcome.classify`; the band rule reasons about both evidence volume and proportional dominance (`s > 3*c` cross-multiplication, no floating point) so that identical net margins (e.g. 4S/1C vs. 100S/97C) can resolve to different bands, and `HIGH` is recoverable after a historical contradiction only through overwhelming (>3:1) proportional dominance, never through volume alone; band-only, no numeric score; persisted as one immutable, append-only `core.diagnostic_confidence_observation` row per probe response scored, written inside `DiagnosticSubmissionService.score`'s existing transaction so a failure rolls back the whole submission; never fed by or fed into mastery computation; no change to V2/V3/V4/V5 selection or H1. | **Proposed** | Gates the H5 confidence PR |
